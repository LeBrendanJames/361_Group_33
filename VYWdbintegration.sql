...
WHERE t1.electionFK = (SELECT electionPK FROM tblElection WHERE electionID = '2018MIDTERM')
...
LEFT JOIN tblResponse t2 ON t1.questionPK = t2.questionFK
LEFT JOIN tblVoterToResponse t3 ON t1.questionPK = t3.questionFK AND t3.voterFK = (SELECT voterFK FROM tblUserLogin WHERE username = 'matt')
WHERE t1.electionFK = (SELECT electionPK FROM tblElection WHERE electionID = '2018MIDTERM') AND t1.QuestionID = '2018MID_Q1'

-- Compact version
SELECT t1.questionID, t1.questionTitle, t1.questionURL, t2.responsePK, t2.responseID, t2.responseTitle, t2.responseSubTitle, t3.responseFK, t3.writeInResponse FROM tblQuestion t1 LEFT JOIN tblResponse t2 ON t1.questionPK = t2.questionFK LEFT JOIN tblVoterToResponse t3 ON t1.questionPK = t3.questionFK AND t3.voterFK = (SELECT voterFK FROM tblUserLogin WHERE username = 'matt') WHERE t1.electionFK = (SELECT electionPK FROM tblElection WHERE electionID = '2018MIDTERM') AND t1.QuestionID = '2018MID_Q1'
...
INSERT INTO tblVoterToResponse (voterFK, questionFK, responseFK, writeInResponse)
  VALUES
  ((SELECT voterFK FROM tblUserLogin WHERE username = 'matt'), (SELECT questionPK FROM tblQuestion WHERE questionID = '2018MID_Q1'), (SELECT responsePK FROM tblResponse WHERE responseID = '2018MID_Q1R1'), NULL)
...
INSERT INTO tblVoterToResponse (voterFK, questionFK, responseFK, writeInResponse)
  VALUES
  ((SELECT voterFK FROM tblUserLogin WHERE username = 'matt'), (SELECT questionPK FROM tblQuestion WHERE questionID = '2018MID_Q2'), NULL, 'Ross Perot')
...
SET    responseFK = (SELECT responsePK FROM tblResponse WHERE  responseID = '2018MID_Q1R2'), 
       writeInResponse = NULL 
WHERE  voterFK = (SELECT voterFK FROM tblUserLogin WHERE username = 'matt') 
       AND questionFK = (SELECT questionPK FROM tblQuestion WHERE  questionID = '2018MID_Q1') 
...
WHERE  voterFK = (SELECT voterFK FROM tblUserLogin WHERE username = 'matt') 
       AND questionFK = (SELECT questionPK FROM tblQuestion WHERE  questionID = '2018MID_Q2') 
